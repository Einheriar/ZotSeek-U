...
zotseek-pref-indexModeMismatchAction = Klicken Sie unten auf „Index neu erstellen“, um die neue Einstellung des Indexmodus anzuwenden.
...
zotseek-pref-mcpServerLabel =
    .label = KI-Agenten die Suche in Ihrer Bibliothek erlauben (lokaler MCP-Server)
zotseek-pref-mcpServerDesc = Ermöglicht MCP-Clients wie Claude Code schreibgeschützte semantische Suchen in Ihrer Bibliothek. Alles bleibt auf diesem Computer (nur localhost).
