...
zotseek-pref-brief-test-consent-title = Verbindung für Literaturzusammenfassungen testen?
zotseek-pref-brief-test-consent-message = Dieser Test sendet nur einen festen Testtext an { $provider } und kann geringe API-Kosten verursachen. Arbeiten, Notizen oder Dateipfade werden nicht gesendet. Fortfahren?
zotseek-pref-brief-consent-message = Für eine Zusammenfassung sendet ZotSeek den Titel, das Abstract und den Text der PDF-Seiten an { $provider }. Beim Erstellen von Prompt-Vorlagen werden die beiden Vorlagen und Ihre Formulareingaben gesendet. Für Ihr BYOK-Konto können Kosten anfallen; auch ein Verbindungstest kann geringe Kosten verursachen. Fortfahren?
...
zotseek-brief-usage-heading = Vom Anbieter gemeldete Token-Nutzung:
zotseek-brief-usage-input = Eingabe: { $tokens }
zotseek-brief-usage-output = Ausgabe: { $tokens }
zotseek-brief-usage-reasoning = Reasoning: { $tokens } (separat ausgewiesen; nicht erneut addiert)
zotseek-brief-usage-total = Gesamt: { $tokens }
zotseek-brief-usage-total-unavailable = Gesamt: Anbieter hat keinen verwendbaren Wert zurückgegeben
zotseek-brief-usage-requests = Verwendbare Nutzung zurückgegeben: { $reported } von { $total } Anfragen
zotseek-brief-usage-incomplete = Für { $count } weitere Anfrage(n) wurde keine verwendbare Nutzung zurückgegeben. Die Angaben können unvollständig sein und entsprechen nicht der Endabrechnung.
...
zotseek-brief-preparation-changed = Anbieter, Modell oder Ausgabelimit haben sich während der Vorbereitung geändert. Starten Sie die Erstellung erneut und bestätigen Sie noch einmal.
zotseek-brief-generation-confirm-title = Senden und Erstellen der Literaturzusammenfassungen bestätigen?
zotseek-brief-generation-confirm-message = Zusammenfassungen für { $count } Arbeit(en) erstellen und Titel, Abstracts sowie PDF-Seitentext an { $provider } senden (Modell: { $model }). Geschätzte Eingabe: etwa { $inputTokens } Tokens; Ausgabelimit je Anfrage: { $outputTokens } Tokens; mindestens erwartet: { $requests } Anfrage(n). Die Schätzung dient nur zur Orientierung. Für das BYOK-Konto können Kosten entstehen; maßgeblich sind Anbieterangaben und Abrechnung. Fortfahren?
