...
zotseek-pref-indexModeMismatchAction = Нажмите «Перестроить индекс» ниже, чтобы применить новую настройку режима индексирования.
...
zotseek-pref-mcpServerLabel =
    .label = Разрешить AI-агентам искать в вашей библиотеке (локальный MCP-сервер)
zotseek-pref-mcpServerDesc = Позволяет MCP-клиентам, например Claude Code, выполнять семантический поиск в библиотеке только для чтения. Всё остаётся на этом компьютере (только localhost).
