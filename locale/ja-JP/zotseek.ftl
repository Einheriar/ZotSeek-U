...
zotseek-pref-brief-test-consent-title = 文献ブリーフの接続をテストしますか？
zotseek-pref-brief-test-consent-message = このテストでは固定テスト文のみを { $provider } に送信し、少額の API 料金が発生する場合があります。論文、ノート、ファイルパスは送信しません。続行しますか？
zotseek-pref-brief-consent-message = ブリーフを生成するため、ZotSeek は論文のタイトル、要旨、PDF ページのテキストを { $provider } に送信します。プロンプトテンプレートの作成時には、2 つのテンプレートとフォームの回答を送信します。BYOK アカウントに料金が発生する場合があり、接続テストにも少額の料金が発生する可能性があります。続行しますか？
...
zotseek-brief-usage-heading = プロバイダー報告の Token 使用量：
zotseek-brief-usage-input = 入力：{ $tokens }
zotseek-brief-usage-output = 出力：{ $tokens }
zotseek-brief-usage-reasoning = 推論：{ $tokens }（個別表示し、再加算しません）
zotseek-brief-usage-total = 合計：{ $tokens }
zotseek-brief-usage-total-unavailable = 合計：プロバイダーから利用可能な値が返されませんでした
zotseek-brief-usage-requests = 使用量取得済み：{ $reported } / { $total } リクエスト
zotseek-brief-usage-incomplete = さらに { $count } 件のリクエストで使用量が返されませんでした。数値は不完全な可能性があり、最終請求額とは一致しない場合があります。
...
zotseek-brief-preparation-changed = 準備中にプロバイダー、モデル、または出力上限が変わりました。生成をやり直して再確認してください。
zotseek-brief-generation-confirm-title = 送信して文献ブリーフを生成しますか？
zotseek-brief-generation-confirm-message = { $count } 件の論文についてブリーフを生成し、タイトル、抄録、PDF ページテキストを { $provider }（モデル：{ $model }）へ送信します。推定入力：約 { $inputTokens } tokens、1 リクエストの出力上限：{ $outputTokens } tokens、予想最少リクエスト数：{ $requests }。推定値は参考情報です。BYOK アカウントに料金が発生する場合があり、最終的な使用量と請求はプロバイダー情報に従います。続行しますか？
