...
zotseek-pref-brief-test-consent-title = ¿Probar la conexión de resúmenes bibliográficos?
zotseek-pref-brief-test-consent-message = Esta prueba solo envía texto fijo a { $provider } y puede generar un pequeño cargo de API. No envía artículos, notas ni rutas de archivos. ¿Continuar?
zotseek-pref-brief-consent-message = Para generar un resumen, ZotSeek enviará a { $provider } el título, el resumen y el texto de las páginas PDF del artículo. Al crear plantillas de prompts, enviará las dos plantillas y tus respuestas del formulario. Tu cuenta BYOK puede incurrir en cargos, y una prueba de conexión también puede tener un pequeño coste. ¿Continuar?
...
zotseek-brief-usage-heading = Uso de tokens informado por el proveedor:
zotseek-brief-usage-input = Entrada: { $tokens }
zotseek-brief-usage-output = Salida: { $tokens }
zotseek-brief-usage-reasoning = Razonamiento: { $tokens } (se muestra por separado; no se vuelve a sumar)
zotseek-brief-usage-total = Total: { $tokens }
zotseek-brief-usage-total-unavailable = Total: el proveedor no devolvió un valor utilizable
zotseek-brief-usage-requests = Uso utilizable devuelto: { $reported } de { $total } solicitudes
zotseek-brief-usage-incomplete = { $count } solicitud(es) adicional(es) no devolvieron uso utilizable. Las cifras pueden estar incompletas y no equivalen a la factura final.
...
zotseek-brief-preparation-changed = El proveedor, el modelo o el límite de salida cambió durante la preparación. Inicia de nuevo la generación y vuelve a confirmar.
zotseek-brief-generation-confirm-title = ¿Confirmar el envío y la generación de resúmenes?
zotseek-brief-generation-confirm-message = Se generarán resúmenes para { $count } artículo(s) y se enviarán títulos, resúmenes y texto de páginas PDF a { $provider } (modelo: { $model }). Entrada estimada: unos { $inputTokens } tokens; límite de salida por solicitud: { $outputTokens }; mínimo previsto: { $requests } solicitud(es). La estimación es orientativa. La cuenta BYOK puede generar cargos; prevalecen el uso y la factura del proveedor. ¿Continuar?
