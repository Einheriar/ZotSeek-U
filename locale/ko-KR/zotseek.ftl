...
zotseek-pref-indexModeMismatchAction = 새 색인 모드 설정을 적용하려면 아래의 “색인 재구성”을 클릭하세요.
...
zotseek-pref-mcpServerLabel =
    .label = AI 에이전트의 라이브러리 검색 허용 (로컬 MCP 서버)
zotseek-pref-mcpServerDesc = Claude Code와 같은 MCP 클라이언트가 라이브러리에서 읽기 전용 의미 검색을 실행할 수 있게 합니다. 모든 데이터는 이 컴퓨터에서만 처리됩니다 (localhost 전용).
