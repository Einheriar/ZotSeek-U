...
zotseek-brief-wizard-template-bundled-desc = 内置模板面向心理学、发展心理学和认知神经科学，默认生成中文简报；选择后也会将两份模板保存到默认下载目录。
zotseek-brief-wizard-bundled-failed = 内置模板下载或启用失败，当前提示词设置未更改。
...
zotseek-brief-wizard-bundled-success = 已启用内置双模板，并将两份模板保存到默认下载目录。以后可以从设置页重新进入引导或导入自定义模板。
zotseek-brief-wizard-progress = 第 { $current } / { $total } 步
zotseek-brief-wizard-step-intro = 介绍
zotseek-brief-wizard-step-service = 服务
zotseek-brief-wizard-step-template = 模板
zotseek-brief-wizard-step-questions = 需求
zotseek-brief-wizard-step-confirm = 确认
zotseek-brief-wizard-intro-title = 什么是文献简报？
zotseek-brief-wizard-intro-lead = 简报由大模型通读论文 PDF 后撰写：它不是摘要的压缩，而是理解的重建，把术语、数据和模型密集的论文转述成一篇凝练、准确、可溯源的学术简报，保存在条目下作为子笔记。
zotseek-brief-wizard-intro-readable-title = 高密度，且高度人类可读
zotseek-brief-wizard-intro-readable-desc = 固定的章节结构（核心问题 → 方法 → 发现 → 讨论）完整保留研究逻辑、关键数值与原始引用线索，一杯咖啡的时间读懂一篇论文。
zotseek-brief-wizard-intro-search-title = 写给人读，也变成更好的检索入口
zotseek-brief-wizard-intro-search-desc = 简报子笔记会进入 ZotSeek 的笔记索引。结构化、术语双语对齐的正文，让你在语义与混合搜索中直接命中论文的方法与发现——而不只是标题和摘要。
zotseek-brief-wizard-intro-privacy-title = 隐私与安全可控
zotseek-brief-wizard-intro-privacy-desc = 功能默认关闭；每次生成前明确确认并显示预计 token 消耗，确认前不发出任何请求；你的阅读笔记、批注永远不会被读取或外发。
