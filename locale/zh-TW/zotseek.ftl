...
zotseek-brief-wizard-template-bundled-desc = 內建範本面向心理學、發展心理學和認知神經科學，預設生成中文簡報；選擇後也會將兩份範本儲存到預設下載目錄。
zotseek-brief-wizard-bundled-failed = 內建範本下載或啟用失敗，目前的提示詞設定未變更。
...
zotseek-brief-wizard-bundled-success = 已啟用內建雙範本，並將兩份範本儲存到預設下載目錄。以後可以從設定頁重新進入引導或匯入自訂範本。
