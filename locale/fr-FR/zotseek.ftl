...
zotseek-pref-brief-test-consent-title = Tester la connexion des synthèses bibliographiques ?
zotseek-pref-brief-test-consent-message = Ce test envoie uniquement un texte fixe à { $provider } et peut entraîner un faible coût d’API. Il n’envoie ni article, ni note, ni chemin de fichier. Continuer ?
zotseek-pref-brief-consent-message = Pour générer une synthèse, ZotSeek enverra à { $provider } le titre, le résumé et le texte des pages PDF de l’article. Lors de la création de modèles de prompts, les deux modèles et vos réponses au formulaire seront envoyés. Votre compte BYOK peut entraîner des frais ; même un test de connexion peut occasionner de faibles frais. Continuer ?
...
zotseek-brief-usage-heading = Utilisation des tokens signalée par le fournisseur :
zotseek-brief-usage-input = Entrée : { $tokens }
zotseek-brief-usage-output = Sortie : { $tokens }
zotseek-brief-usage-reasoning = Raisonnement : { $tokens } (affiché séparément, sans nouvelle addition)
zotseek-brief-usage-total = Total : { $tokens }
zotseek-brief-usage-total-unavailable = Total : aucune valeur exploitable renvoyée par le fournisseur
zotseek-brief-usage-requests = Utilisation exploitable renvoyée : { $reported } requête(s) sur { $total }
zotseek-brief-usage-incomplete = { $count } requête(s) supplémentaire(s) n’ont renvoyé aucune utilisation exploitable. Les chiffres peuvent être incomplets et ne constituent pas la facture finale.
...
zotseek-brief-preparation-changed = Le fournisseur, le modèle ou la limite de sortie a changé pendant la préparation. Relancez la génération et confirmez à nouveau.
zotseek-brief-generation-confirm-title = Confirmer l’envoi et la génération des synthèses ?
zotseek-brief-generation-confirm-message = Générer des synthèses pour { $count } article(s) et envoyer leurs titres, résumés et textes de pages PDF à { $provider } (modèle : { $model }). Entrée estimée : environ { $inputTokens } tokens ; limite de sortie par requête : { $outputTokens } ; minimum prévu : { $requests } requête(s). L’estimation est indicative. Le compte BYOK peut être facturé ; l’utilisation et la facture du fournisseur font foi. Continuer ?
...
zotseek-search-itemNotFound = L’élément n’existe pas
