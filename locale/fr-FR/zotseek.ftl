...
zotseek-pref-indexModeMismatchAction = Cliquez sur « Reconstruire l’index » ci-dessous pour appliquer votre nouveau réglage du mode d’indexation.
...
zotseek-pref-mcpServerLabel =
    .label = Autoriser les agents IA à rechercher dans votre bibliothèque (serveur MCP local)
zotseek-pref-mcpServerDesc = Permet aux clients MCP tels que Claude Code d’effectuer des recherches sémantiques en lecture seule dans votre bibliothèque. Tout reste sur cet ordinateur (localhost uniquement).
