...
本文档说明 ZotSeek 的语义检索与 Hybrid 混合搜索。2026-09-10 的文献级实现已合入 main 并推送（`211b22e`），以“当前文献级 Hybrid”一节为准；历史实验图和旧 RRF 示例不代表当前默认算法。
...
当前搜索架构分为两层：索引模式决定可搜索的内容范围，搜索模式决定使用哪些召回通道。文献级 Hybrid 是默认入口；它先尝试身份导航，失败后并行执行语义检索和关键词检索，再用固定的有界关键词奖励排序。三个索引模式共用同一文献级公式，Full 不再为 Notes 或 PDF 预留固定名额。
...
显式选择语义模式时绕过 Hybrid 身份导航与融合，直接执行语义搜索。
...
quick search **没有被移除**：它继续负责元数据面的检索，与 BM25 的命中按条目合并成"关键词分支"，再与语义侧做受控分数融合。所以今天的 Hybrid 实际上是三套机制的糅合——语义嵌入 + Zotero 元数据关键词 + 正文 BM25——外加最前面的身份导航。
...
- **冷构建延迟**：Plan 40B 的旧版 150 篇库数据不能外推大库。Plan 62 在启动维护末尾准备，快照命中避免重新分词。format 3 使用单文件分段 JSON，编解码与校验约每 8 ms 检查是否让出主线程；单条记录、字符串合并、文件 I/O、校验和与最终字典恢复仍有同步成本，不保证严格帧时间。58,105 chunks 的隔离实测：命中约 6.85 s，最大事件循环间隔约 0.52 s；从零准备明显更长，峰值内存必须按整进程另行衡量。
...
RRF 根据各路名次合并列表，不需要把原始分数归一到同一尺度，但 k 与权重仍是设计参数。下图是旧版 S/K RRF 的教学示例；当前文献级 Hybrid 只在 Q/L 形成 K50 时使用 RRF。
...
### 当前文献级 Hybrid（Plan 66P）

明确身份导航未命中时，三个索引模式共用下列合同。S 是所选范围内所有有效向量块的文献 MaxSim；并不是先由关键词筛文献，再算语义。
...
首次向量缓存解码每 16 块检查约 8 ms 时间预算，超预算用定时器让出事件循环。归一化运算顺序不变；generation/model/lifecycle 检查仍在发布前执行，并发搜索共享构建。它不是后台线程，也不保证整个加载过程无停顿。隔离 57,523 块实测最长计时器间隔由 1.725 s 降到 0.763 s，总加载由 5.808 s 变为 6.344 s；这些是单轮组件对比，不是完整 UI 延迟或稳定性能保证。

冻结 66L 的 1,080 个单元通过生产融合函数重放；180 个有完整 Q/L 输入的单元通过 K50 重放。该结果验证公式接线，不重新证明跨库效果，也不包含真实检索候选的完全等价证明。
...
- 摘要模式上词法证据过弱：该历史实验中的旧 Hybrid 前排（R@1 0.06）低于纯语义（0.22）。这只说明当时的候选和旧公式，不代表当前实现把 `abstract` 文献级 Hybrid 改成 semantic-only。
