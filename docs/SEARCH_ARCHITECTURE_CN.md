...
本分支正在替换后文 Plan 40B 的 lexical 即时失效合同：普通 chunks 写入仅使持久化修订号前进，已就绪 BM25 允许在当前会话保持旧内容；向量缓存仍即时失效。首次准备尝试独立 JSON 快照，匹配失败再构建；模型切换、清空、重连和关闭保持强失效。修订号与写入事务原子提交，JSON 内容校验不依赖全库文本扫描。分词与评分顺序不变，构建分批让出主线程。启动/手动维护和旧条目错误将在本计划后续步骤补齐，真实运行时尚待验收。
...
- **进程内存**：倒排索引（词项表 + postings）构建后常驻内存，同规模语料约 66,532 个词项、967,798 条 postings。缓存随 Zotero 退出丢弃，不持久化。
- **冷构建延迟**：首次词法查询或缓存失效后的重建约 4.4–6.5 秒（Plan 40B 实测）；期间并发查询合并到同一次构建，索引持续写入期间按安全降级策略暂时省略词法证据（见[性能优化](#性能优化)）。
...
- 索引**进程内、非持久化**：每个活动模型分区构建一次，Zotero 退出即丢，下次查询重建。
- 与向量缓存共享同一个 mutation generation，但各自持有 keyed single-flight：并发冷查询合并到同一次构建；发布前复核 generation、活动模型与 store 生命周期；过期构建丢弃并最多重试一次。
- 索引持续写入期间按安全降级策略暂时省略索引侧词法证据，绝不返回已知过期的混合结果（详见[性能优化](#性能优化)）。
...
搜索使用三个进程内、非持久化的缓存。语义缓存保存所有已存储模型分区的预归一化向量加轻量来源与位置元数据；它刻意排除 `chunk_text`。自 Plan 43 起，向量缓存只读取活动模型和该窄投影，把存储向量直接解码为 `Float32Array` 并原地归一化。公开的 `getAll()` 批量契约保持完整且跨模型。T0 词法缓存为活动模型/回退语料保存一个内存 BM25 索引。两个缓存都在 Zotero 退出时丢失，因此启动或失效后的第一个对应查询会从 `zotseek.sqlite` 重建它们。
...
两个缓存共享单调递增的 mutation generation，但使用独立的 keyed single-flight 构建。同一 generation 的并发冷语义查询共享一次向量读取；同一模型与 generation 的并发词法查询共享一次语料读取与分词。文献库与来源限制在搜索共享基础索引时应用；它们不会创建按库或按来源的完整缓存副本。

构建只有在完成时其 generation、活动模型（如适用）与 store 生命周期仍然有效才发布。成功的索引写入、清空、模型删除、数据库重新挂载、压缩或 store 关闭都会先使发布资格失效。如果构建过期则丢弃并重试一次。因此持续索引期间可能暂时缺少索引侧的 BM25 证据；反复失效的向量构建保留其错误语义，由 Hybrid 层转换为空语义分支。绝不返回已知过期或可能混杂的缓存。写入平息后的下一个查询会正常重建，无需重启或清空索引。
