...
When a result with a page is activated, the UI passes `pdf_attachment_key` to the reader entry point and opens only that PDF under the same parent item and library. Only legacy indexed results without provenance use `getBestAttachment()` as a compatibility fallback; if an exact recorded attachment was deleted or no longer matches, ZotSeek does not silently open a different PDF.
...
3. Apply References v2 region filtering, followed by repeated page-furniture
   ...
   The separate MCP/REST and Brief `readPdfAttachment` batch path rereads mismatched batches sequentially, one physical page at a time, preserving empty-page positions without guessing or padding boundaries. The worker's final `trim()` can remove boundaries around empty edge pages. This does not change index extraction, chunking or cleanup described here.
...
Full extraction and incremental Child Note replacement use the same quota
allocator, so the retained Summary/Note/PDF counts and `wasTruncated` decision
do not vary by entry point. If an existing Full item is already marked
truncated, a later Note-content change takes the whole-item rebuild path and
re-extracts its PDF. This lets PDF chunks refill slots released by Note changes
and recomputes page coverage and truncation state from the resulting item.
...
- **`needsReindexByIdentity(libraryKey, itemKey, contentHash, modelId)`** — compares the new hash with `item_models.content_hash` for the specified model. The shared `items.content_hash` is retained only for legacy-schema/migration compatibility and cannot make model B appear current merely because model A was refreshed.
...
When an in-process ChromeWorker crashes, all embedding calls rejected at the same time share one recovery task and one replacement Worker. Late events are filtered by Worker instance identity, so an old Worker cannot fail the replacement or complete its jobs. Reset/destroy cancels unfinished initialization, preventing recovery from reviving a Worker after its lifecycle has ended.
...
**Failure semantics:** `ServerEmbeddingClient.embed()` retries network errors, timeouts and 5xx responses with a bounded backoff of 2s, 5s, then 15s; a 4xx response fails immediately (a configuration problem that retrying cannot fix). Every successful HTTP response must still contain a complete unique `index` set and, for each input, an array of exactly the configured dimensions containing only finite values and not all zeroes. An invalid 200 response fails immediately with `SERVER_INVALID_RESPONSE`; it is neither retried nor stored. After the retry budget is exhausted, the client throws `ServerUnavailableError`, which propagates out of `embedChunks()` to the caller's outer catch and stops the run cleanly, the same way a cancellation does. There is deliberately **no fallback to the in-process ONNX model**: silently switching runtimes mid-run would mix two different vector spaces under one `model_id`. Update Index resumes from the same checkpoint mechanism used for any interrupted run once the server is back.
...
Each provider has a dedicated request adapter behind one shared transport client. The Bailian adapter keeps the native DashScope `text-embedding` request (`/api/v1/services/embeddings/text-embedding/text-embedding`, `parameters.text_type`, `output_type=dense`) while the persisted shared Base URL stays on `/compatible-mode/v1`. The OpenAI adapter posts `{model, input, dimensions}` with Bearer auth and sends the same body for query and document kinds (no task roles, no textual prefixes). The Gemini adapter posts `batchEmbedContents` with `x-goog-api-key`, one request per text, placing `taskType` (`RETRIEVAL_QUERY`/`RETRIEVAL_DOCUMENT`) and `outputDimensionality` directly on each request, matching the Google SDK and the verified 768-dimensional runtime response (`gemini-embedcontent-v2`). Cloud roles are provider API parameters rather than E5-style string prefixes; the adapter version, document-side role and per-provider output contract are part of the document index policy fingerprint, while the query role only affects future queries.

Literature briefs follow the last Cloud provider saved in preferences and reuse that provider's credential and endpoint; choosing the local or loopback-server runtime for Embedding does not prevent briefs from using the saved Cloud provider. The brief generation model is independent from the Embedding model and saved per provider, with a single free-text field plus discovered suggestions. Bailian/Gemini suggestions can be filtered by capability metadata, while OpenAI/Custom `/models` entries are unverified hints; every choice must still pass the brief-specific connection test. Generation adapters use Bailian/Custom Chat Completions, the OpenAI Responses API, and Gemini `generateContent`, respectively. External-data consent and connection verification are isolated per provider, so changing the data recipient requires separate consent; changing a provider credential, endpoint, or matching generation config invalidates only that provider's brief verification.
...
**Recovery of stranded legacy databases:** startup does not trust `schema_version` alone; it also checks migration-defining structures. If the physical layout still contains the v3 `embeddings` table, or `items` still lacks the v8 `library_key`, while an historical build has recorded a newer version, storage lowers the replay point to the structurally verified v3, v6, or v7 boundary and reruns the idempotent migrations in order. Current v8+ layouts do not enter this downgrade path. Existing v8/v9 backups and transaction boundaries remain in force, so a failed migration can be retried from the original database.
...
The UI requires these numeric preferences to be non-empty integers within the control range, while core readers still defend against historical or external writes. `maxChunksPerPaper` is truncated and clamped consistently to 1–200, with non-numeric values falling back to 100; extraction, Full-source allocation, incremental Note replacement, and freshness fingerprints share that result. A non-numeric `maxTokens` or a value below 50 falls back to the active model recommendation; result count and similarity percentage are clamped to 5–100 and 0–100 respectively.
