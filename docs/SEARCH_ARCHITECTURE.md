...
Cloud roles are provider API parameters rather than E5-style string prefixes. The Bailian adapter fixes the parameter name as `text_type`, sends each non-empty role value in `parameters`, and omits the parameter when its configured value is empty. It sends raw prepared text, explicitly requests `output_type=dense`, leaves `instruct` unset, and never falls back to a manual prefix when the provider rejects a role. Query and document inputs are never mixed in one request. Changing either role clears connection verification; the adapter version, document role, dense output and absent instruction are part of the document index policy fingerprint, while the query role only affects future queries.

The connection probe submits one fixed document string and one fixed query string in separate requests. Native responses are accepted only when every input has one unique `text_index` and a non-zero vector containing exactly the configured number of finite values. Network errors, 429 and 5xx responses use bounded retries; deterministic 4xx responses fail immediately. Provider response bodies are reduced to a bounded safe error code, so text, queries and credentials are never copied into logs or user errors.
