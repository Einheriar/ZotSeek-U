...
**Incomplete-selection behavior:** selecting an incomplete Server slot is allowed and survives restart, but it never falls back to an ONNX model. Clicking the slot or explicitly starting indexing, semantic/hybrid search or similar-document search shows a configuration error with the template path. MCP/REST returns the same error as text. Startup and background reconciliation do not show a modal and skip work that needs embeddings; keyword-only search remains available. A damaged Server template is inert when a local model is selected.
