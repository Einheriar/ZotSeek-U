...
2026-09-09：已接入 JSON 快照基础、数据库语料修订号与分批构建。修订号在 chunks 写入事务内递增，普通写入保留会话内 BM25，清空/模型删除/关闭/重连仍使内存失效。快照匹配数据库身份、修订号、模型和算法合同，另校验 SHA-256；损坏或 IO 失败回退重建，不影响数据库。启动和手动维护衔接、缺失条目提示及真实 Zotero 验收仍在进行中。软件版本改变本身不使快照作废。
