...
2026-09-09：已接入独立 JSON 快照、事务内语料修订号、分批构建、启动末尾准备和现有手动更新入口。普通写入保留会话内 BM25，清空/模型删除/关闭/重连仍使内存失效。快照匹配数据库身份、修订号、模型和算法合同，另校验 SHA-256；软件版本变化本身不使其失效。准备期间搜索等待共享任务，无双版本服务；缺失条目返回 item_not_found，中文 UI 显示“条目不存在”，MCP/REST 不返回该条目的链接或实时元数据。十种语言包同步。Node 测试及冻结九组语料排名/JSON 往返通过，真实 Zotero 验收进行中；JSON 同步解析/序列化与峰值内存仍需实测，不承诺二进制或固定启动秒数。
