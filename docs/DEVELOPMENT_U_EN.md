...
2026-09-11 Plan 73: MCP/REST `search`, `find_similar`, and `get_item` can add optional `journalMetrics` from an already loaded Zotero Style journal cache. `impactFactor` maps `sciif`, while `sciQuartile` accepts only JCR SCI `Q1`–`Q4`. For Style 6.x the adapter reads `rank` entries keyed by publication title from `zoterostyle.json` in the Zotero data directory, retains an in-memory-cache fallback for older releases, and uses Zotero's Add-on Manager to verify that Style is active when extension compartment isolation hides its runtime global. A result set shares a short-lived read snapshot. The adapter never reads credentials, calls EasyScholar, or invokes Style's column data provider, which could schedule a network refresh. Missing addons, cache entries, or valid values omit the field, while third-party shape or file-read failures never fail the original request. Metrics do not participate in metadata, indexing, filtering, or ranking.

2026-09-13 Plan 74: MCP/REST `search` can use stable `library_key + collection_key` values to constrain live collection members before candidate TopK; MCP includes all descendants by default and can request direct members only. The new read-only `get_library_map` and `GET /zotseek/library-map` return the complete ordinary-collection tree. Structured year/journal/author filtering now runs after bounded-candidate ranking but before final `max_results`, with an added complete live `tag` condition that is NFC-normalized and case-sensitive. This remains a bounded retrieval operation rather than an exhaustive field query: Hybrid/Semantic/Keyword inspect the existing S50∪K50, S50 and K50 respectively, while passages retain compatibility candidates; filtering never reorders. Collection-resolution or tree-integrity failure is explicit and never widens to the whole library or returns a misleading partial tree. No database schema, indexing policy, embedding, chunk or XPI/manifest contract changed.

On top of upstream's `search` / `find_similar` / `index_status` (full usage in [MCP.md](MCP.md)):

- **`get_library_map` tool**: returns the complete live ordinary-collection tree for a stable `library_key`; each node contains only `collectionKey`, `name` and `children`. Empty collections are included; saved searches, item lists, tags and index-coverage claims are omitted. REST equivalent: `GET /zotseek/library-map`.
- **`get_item` tool**: reads normalized bibliography, tags, collections, relatedItems and attachment lists by `library_key + item_key`; `include_notes: true` returns all child notes' complete unfiltered text (the index-side "basic information"/References exclusion rules are not applied) plus live `sections` / `sectionPaths`; `include_pdf: "pages" | "full" | "references"` supports a chosen attachment, with ≤20 consecutive pages per explicit range, while `full` uses batches of ≤20 pages and returns a leading prefix of at most 100 pages or about 300,000 characters. `references` scans backwards from the document tail under the same batch, page, and approximate character bounds, reuses production References v2, and returns only the detected region with real physical page numbers. A completed scan with no region is `not_found`; a scan-bound miss is `partial`; `referenceDetection` exposes the strategy version and scanned interval. It does not force citations into structured fields or promise a proportional reduction in PDFWorker work. MCP and REST share the contract; reads prefer Zotero's full-text cache and use batched `PDFWorker` fallback for missing pages without claiming that Zotero's underlying queue can be cancelled. Local file paths are never exposed. REST equivalent: `GET /zotseek/item`.
- **`search` scope and structured filtering**: optional `collection_key` searches within a live collection identity set; optional `filter` (`year_from` / `year_to` / `journal` / `author` / `tag` + `exact`) applies to the existing bounded candidates before final `max_results`, without reordering or expanding candidate depth. REST uses the corresponding camelCase parameters.
...
- The About section displays the runtime plugin version passed by bootstrap and links to this fork's repository, `https://github.com/Einheriar/ZotSeek-U`; the version is not hard-coded in XHTML or locale resources.
...
- Setup becomes usable only after an explicit bundled-template choice or a complete customized/imported pair. Choosing the bundled pair in the guide first saves both packaged templates to the default Downloads folder with non-overwriting names, and switches to the bundled source only after both files succeed; a download failure leaves the current prompts and setup state unchanged. Advanced import replaces one slot while preserving the other; personalized files are likewise downloaded before the controlled profile pair is activated.
...
2026-09-11 (Plan 72): the wizard window received a visual refresh and a new intro page. The window now loads a dedicated stylesheet `content/briefPromptWizard.css` (same mounting pattern as `searchDialog.css`): a muted gray-blue gradient banner (inline SVG background art, no new image assets), a dot-based step indicator replacing the old "Step X of 4" text, card-style page containers, and a full `prefers-color-scheme` dark palette. A new intro page opens the wizard and explains the brief feature's design intent (a dense yet highly readable reading aid, a notes-index search entry, and the privacy/safety boundaries) with no form and no requests; the page flow becomes intro → service → template → questions → confirm → result. Business logic (connection test, template choice, generation, cancel semantics) is unchanged. New strings cover zh-CN and en-US; the other eight locales carry English placeholders marked `TODO(translation)`. The runtime layout fix removes an empty spacer that competed with the page for height: only the middle page region grows and scrolls, while the banner, status and navigation keep their own height. Page changes preserve the user's window size instead of automatically resizing, and reset content scrolling. Intro advances explicitly to service without validating later form fields. Offline gates passed (543/543 tests, typecheck baseline, versions, build), as did 10 navigation steps and 72 size/page/theme/text layout cases in Zotero 9.0.6. English uses static text fixtures and the result page is presentation-only; full English UI, dynamic result paths and actual generation acceptance remain incomplete. No model requests or Note writes were performed in this validation.
...
- `src/dev/suites/` is the self-test harness that needs a real Zotero (34 MCP scenarios, model loading, database integrity, etc.) and cannot be replaced by Node mocks.
